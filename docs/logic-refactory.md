...
"Create the Domain Events in the `Domain` layer for the `Cliente` entity (e.g., `ClienteCreatedEvent`, `ClienteUpdatedEvent`, `ClienteDeletedEvent`). These events should implement `INotification` (from MediatR) and carry the `EmpresaId` payload, as the cache keys are isolated by Tenant."

### Task 3: Emit Events from Entities
"Modify the `Cliente` domain entity. Inside the rich constructor, the `AtualizarDados` method, and the `Desativar` method, append the newly created domain events to the base class's `AddDomainEvent()` method."

### Task 4: Implement Cache Invalidation Handlers
"In the `Infrastructure` layer, create a `ClienteCacheInvalidationHandler` that implements `INotificationHandler<ClienteCreatedEvent>`, etc. Inject `IMemoryCache` into this handler, and implement the `Handle` method so that it executes `_cache.Remove($"Clientes_{notification.EmpresaId}")` when triggered."

### Task 5: Implement Cache Lookups in Queries
"Update the `ClienteAppService.cs`. Inject `IMemoryCache`. In the `GetAllAsync` method, check if the key `$"Clientes_{empresaId}"` exists. If it does, return the cached list. If not, fetch from the repository, store the result in the cache with an expiration policy (e.g., 10 minutes), and then return it. Ensure no caching logic is added to the Command methods."
