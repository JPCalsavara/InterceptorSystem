...
public class Contrato : Entity, IAggregateRoot
{
    ...
    {
        Enforce(empresaId != Guid.Empty, "O contrato deve estar associado a uma empresa.");
        Enforce(clienteId != Guid.Empty, "O contrato deve pertencer a um cliente.");
        Enforce(!string.IsNullOrWhiteSpace(descricao), "A descrição do contrato é obrigatória.");
        Enforce(valorTotalMensal > 0, "O valor total mensal deve ser maior que zero.");
        Enforce(valorDiariaCobrada > 0, "O valor da diária deve ser maior que zero.");
        CheckPercentual(percentualAdicionalNoturno, "Percentual de adicional noturno inválido.");
        Enforce(valorBeneficiosExtrasMensal >= 0, "Valor de benefícios não pode ser negativo.");
        Enforce(numeroDePostos >= 1, "Número de postos deve ser maior ou igual a 1.");
        ...
        Enforce(dataFim > dataInicio, "A data final deve ser posterior à data inicial.");
        Enforce(
        ...
        Enforce(Enum.IsDefined(status), "Status do contrato é obrigatório.");
