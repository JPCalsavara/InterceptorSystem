...
public class ContratosController : TenantControllerBase
{
    ...
    public ContratosController(IContratoAppService service, ICurrentTenantService currentTenant) : base(currentTenant)
    {
        _service = service;
    }
    ...
    public async Task<IActionResult> Create(CreateContratoDtoInput input, CancellationToken ct = default)
    {
        try
        {
    ...
    [HttpGet("/api/clientes/{clienteId}/contratos")]
    [ProducesResponseType(typeof(IEnumerable<ContratoDtoOutput>), StatusCodes.Status200OK)]
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<IActionResult> Update(Guid id, UpdateContratoDtoInput input, CancellationToken ct = default)
    {
        try
        {
