...
# Instância base do LLM usando a chave de ambiente
llm = ChatOpenAI(
...
async def process_support_message(message: str, tenant_id: str) -> str:
    ...
    if settings.openai_api_key == "sk-mock-key" or not settings.openai_api_key:
        return (f"[Modo Dev / Sem Chave OpenAI] Olá! Percebi que sua dúvida é: '{message}'. "
                "Para testar o fluxo real, adicione sua OPENAI_API_KEY no arquivo .env do microsserviço Python.")
    
    try:
        # TODO: Implementar busca no banco vetorial ChromaDB ou PGVector
